...
#define CLK_X_DIV_IDX(h616chip, ch) ((h616chip)->data->npwm + (ch))
#define CLK_BYPASS(h616chip, ch) ((h616chip)->data->npwm * 2 + (ch))
#define CLK_XY_SRC_IDX(h616chip, ch) ((h616chip)->data->npwm * 3 + ((ch) >> 1))
...
static int h616_pwm_set_bypass(struct h616_pwm_chip *h616chip, unsigned int idx,
			       bool en_bypass)
{
	...
	struct clk *src_xy_clk, *div_clk;

	src_xy_clk = hw_data->hws[CLK_XY_SRC_IDX(h616chip, idx)]->clk;
	div_clk = hw_data->hws[CLK_X_DIV_IDX(h616chip, idx)]->clk;

	current_parent_clk = clk_get_parent(chan->pwm_clk);

	if (en_bypass && (src_xy_clk != current_parent_clk))
		return clk_set_parent(chan->pwm_clk, src_xy_clk);
			  ...
			  const struct pwm_state *state)
{
	...
	ret = h616_pwm_set_bypass(h616chip, pwm->hwpwm, chan->bypass);
	if (ret) {
		dev_err(pwmchip_parent(chip), "failed to %sset bypass for PWM %d\n",
			chan->bypass ? "" : "un", pwm->hwpwm);
		chan->bypass = !chan->bypass;
		return ret;
	}
					  ...
					  void *data)
{
	...
	err = h616_pwm_set_bypass(h616chip, idx, true);
	if (err) {
		ret_clk = ERR_PTR(err);
		chan->mode = H616_PWM_MODE_NONE;
		pr_err("Unable to set bypass for PWM clock %u\n", idx);
	} else {
		chan->bypass = true;
	}
out:
...
static int h616_pwm_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(h616chip->base))
		return dev_err_probe(dev, PTR_ERR(h616chip->base),
				     "Failed to get PMW base address\n");
